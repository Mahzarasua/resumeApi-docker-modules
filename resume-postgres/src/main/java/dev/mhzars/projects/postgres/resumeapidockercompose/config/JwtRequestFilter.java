...
import static dev.mhzars.projects.postgres.resumeapidockercompose.config.WebSecurityConfig.AUTH_WHITELIST;
...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    private boolean checkWhiteList(String uri) {
        return Arrays.stream(WHITELISTED_PATHS).anyMatch(path -> StringUtils.contains(uri, path)) || Arrays.stream(AUTH_WHITELIST).anyMatch(path -> StringUtils.contains(path, uri));
    }
